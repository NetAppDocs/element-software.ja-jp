...
---
= リモートのネットアップサポートセッションを開始する（Element 12.3以前）
:allow-uri-read: 


[role="lead"]
Element 12.3以前を実行しており、SolidFire オールフラッシュストレージシステムのテクニカルサポートが必要な場合、ネットアップサポートはお客様のシステムにリモートで接続できます。セッションを開始してリモートアクセスを確立するために、ネットアップサポートはお客様の環境へのリバース Secure Shell （ SSH ）接続を確立します。
