...
この手順を実行する前に、を理解しておく必要があります link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["永続ボリューム"] 使用するかどうかを指定します。永続ボリュームはオプションですが、仮想マシン（ VM ）が失われた場合の管理ノードの設定データのリカバリには推奨されます。
...
NOTE: Element 12..1 以降では、手順（ a ） ～ （ e ）が自動的に実行されます。管理ノード 12..1 の場合は、に進みます ,サブステップ (f) 時刻同期の設定を完了します。
