...
* *エンドユーザライセンス契約（EULA）*：管理サービス2.20以降では、NetApp Hybrid Cloud ControlのUIまたはAPIを使用してElementストレージの健全性チェックを実行する前に、EULAに同意して保存する必要があります。
+
...
健全性チェックは、 NetApp Hybrid Cloud Control の UI 、 NetApp Hybrid Cloud Control API 、または HealthTools スイートを使用して実行できます。
