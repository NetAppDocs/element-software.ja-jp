...
+
vCenter Server 5向けElementプラグインで始まる：0

+
image::vcp_remote_plugin_registration_ui.png[は、プラグインの登録ユーティリティの UI です]

+
vCenter Server 4.10以前のElementプラグイン：
...
+
Element Plug-in for vCenter Server 5.0以降では、NetApp Element リモートプラグイン拡張ポイントは次のとおりです。

+
image::vcp_remote_plugin_icons_home_page.png[は、Element Plug-in 5.10以降のアップグレードまたはインストールが正常に完了したあとのプラグイン拡張ポイントを示しています]

+
Element Plug-in for vCenter Server 4.10以前のバージョンでは、NetApp Element Configuration and Management拡張ポイントは次のとおりです。
